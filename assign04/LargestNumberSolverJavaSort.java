...
public class LargestNumberSolverJavaSort {
	...
	public static Integer[] findKthLargest(List<Integer[]> list, int k) throws IllegalArgumentException {
		...
		ArrayList<BigInteger> bigList = new ArrayList<BigInteger>(list.size());
		
		for (int i = 0; i < list.size(); i++) {
			bigList.add(i, (findLargestNumber(list.get(i))));
		}
		
		ArrayList<BigInteger> bigListCopy = (ArrayList)bigList.clone();
		Collections.sort(bigList, (lhs, rhs) -> {return -lhs.compareTo(rhs);});
		
		for (int i = 0; i < bigList.size(); i++) {
			if (bigListCopy.get(i).equals(bigList.get(k))) {
				return list.get(i);
			}
		}
