 ...
 * Author: Ben Jones and Tyler C. Wilcox
 */
public class GrayscaleImage {
    ...
    public double getPixel(int x, int y){
    	// Check if pixel address is invalid
    	if ((y < 0 || x < 0) || (y >= imageData.length || x >= imageData[y].length)) {
    		throw new IllegalArgumentException("Pixel location is out of bounds.");
    	}
    ...
    /**
     * Gets the height of the image in pixels for comparison
    ...
    /**
     * Gets the width of the image in pixels for comparison
    ...
    public boolean equals(Object other){
        ...
        if (getHeight() != otherImage.getHeight() || getWidth() != otherImage.getWidth()) {
        	return false;
        }
        
        // Check if each pixel has the same value
        for (int row = 0; row < imageData.length; row++) {
        	for (int col = 0; col < imageData[0].length; col++) {
        		if (imageData[row][col] != otherImage.getPixel(col, row)) {
        			return false;
        		}
        	}
        }
        
        return true; // Only returns true if all checkpoints have been passed
    }
    ...
    public double averageBrightness(){
        // Check if imageData is empty and return 0 if so
    	if (imageData.length == 0 || imageData[0].length == 0) {
    		return 0.0;
    	}
    	
    	// Sum all brightness values and divide by pixel count
    ...
    public GrayscaleImage normalized(){
    	// Create new data array with the same dimensions as the current array
    	double[][] newImageData = new double[imageData.length][imageData[0].length];
    	
    	// Check if image is nearly black (will result in divide by 0 error)
    	if (averageBrightness() < 0.1) {
    		// Add 127 to all values
    		for (int row = 0; row < imageData.length; row++) {
    			for (int col = 0; col < imageData.length; col++) {
    				newImageData[row][col] = imageData[row][col] + 127.0;
    			}
    		}
    	}
    	else {
            // Store inverse of average for efficiency
        	double invAvg = 1/averageBrightness();
            // Adjust each pixel's brightness by a factor of 127/average to make the average 127
            for (int row = 0; row < imageData.length; row++) {
            	for (int col = 0; col < imageData[row].length; col++) {
            		newImageData[row][col] = imageData[row][col]*127*invAvg;
            	}
           	}
    	}
        
        // This method of normalization could result in weird values that are outside the bounds established
        // for this class. Also, if too many pixels are black this method will always fail.
    ...
    public GrayscaleImage mirrored(){
        ...
        // Loop from left to right in the current image, storing the values from right to left in the new array
        for (int row = 0; row < imageData.length; row++) {
        	for (int col = 0; col < imageData[0].length; col++) {
        		newImageData[row][imageData[0].length - 1 - col] = 
        				imageData[row][col];
        	}
        }
    ...
    public GrayscaleImage cropped(int startRow, int startCol, int width, int height){
    	// Check if box's top-left or bottom-right corner is out of bounds of the original array
    	if ((startRow < 0 ||
    			startCol < 0) ||
    			startRow + height > imageData.length ||
    			startCol + width > imageData[0].length) {
    		throw new IllegalArgumentException("Cropped area is outside the bounds of the original image.");
    	}
    	
    	// Check if width or height would create a new image with a 0-sized array
    	if (width == 0 || height == 0) {
    		throw new IllegalArgumentException("Final image cannot have an empty dimension.");
    	}
    	
    	// Check if the crop would result in a negatively sized array
    	if (width < 0 || height < 0)
    		throw new IllegalArgumentException("Selected dimensions would result in negatively sized image.");
    	
    	// Create a new array with the desired dimensions
        double[][] croppedData = new double[height][width];
        
        // Copy the data in the given region into the new array
    	for (int row = startRow; row < startRow + height; row++) {
    		for (int col = startCol; col < startCol + width; col++) {
    			croppedData[row - startRow][col - startCol] = 
    					imageData[row][col];
    		}
    	}
    ...
    public GrayscaleImage squarified(){
    	...
    	if (imageData.length > imageData[0].length) {
    		squaredData = new double[imageData[0].length][imageData[0].length];
    		
    		// Subtraction amount (rounded down)
    		subtract = (imageData.length - imageData[0].length)/2;
    		
    		// Fill out new array from the correct spot
    		for (int row = 0; row < squaredData.length; row++) {
    			for (int col = 0; col < squaredData[0].length; col++) {
    				squaredData[row][col] = imageData[row + subtract][col];
    			}
    		}
    	}
    	// Else width is larger than height
    	else {
    		squaredData = new double[imageData.length][imageData.length];
    		
    		// Subtraction amount (rounded down)
    		subtract = (imageData[0].length - imageData.length)/2;
    		
    		// Loop from top down and right to left to fill out new array
    		for (int row = 0; row < squaredData.length; row++) {
    			for (int col = 0; col < squaredData[0].length; col++) {
    				squaredData[row][col] = imageData[row][col + subtract];
    			}
    		}
    	}
